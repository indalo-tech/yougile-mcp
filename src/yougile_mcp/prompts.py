...
1. yougile_client_copies({_args(board=board)}): открытые внутренние задачи — со связью (linked) и без (unlinked), задачи клиентской доски и правила клиентских текстов (rules). Если клиентские копии не настроены, скажи, где их включить, и остановись.
2. Для каждой задачи из unlinked реши:
   - «внутренняя работа» — рефакторинг, CI, инфраструктура, уборка, эксперименты, разбор логов без результата для клиента — копия не нужна;
   - «уже есть копия» — на клиентской доске есть задача о том же (похожий смысл, план часов, колонка): это старая пара, сделанная вручную, её не трогай и не копируй задачу снова;
   - «нужна копия» — работа по заказу клиента, которой на клиентской доске ещё нет.
3. Для «нужна копия» открой задачу через yougile_task и напиши клиентский заголовок и описание строго по rules: без внутренних кодов, технических деталей и личных данных.
4. Покажи таблицу «задача — решение — клиентский заголовок — описание» и спроси, что создавать. Только после явного согласия вызывай yougile_client_copy для выбранных задач (клиентский проект видят клиенты).

Задачи со связью не трогай: их колонка, часы и срок уже следуют за внутренней задачей сами."""
